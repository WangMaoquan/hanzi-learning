...
<template>
  <div>
    ...
    <template v-else>
      <!-- 当前学习 -->
      <div v-if="currentCharacter" class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-6">
        ...
        <div class="flex items-center gap-8">
          <div class="w-40 h-40 flex items-center justify-center bg-primary-50 rounded-xl">
            <span class="text-8xl font-bold text-gray-900">{{ currentCharacter.word }}</span>
          </div>
          <div class="flex-1">
            <div class="mb-4">
              <span class="text-gray-500 text-sm">拼音</span>
              <p class="text-xl text-gray-900">{{ currentCharacter.pinyin || '-' }}</p>
            </div>
            <div class="mb-4">
              <span class="text-gray-500 text-sm">释义</span>
              <p class="text-gray-900">{{ currentCharacter.explanation || '-' }}</p>
            </div>
            <div class="flex gap-4">
              <div>
                <span class="text-gray-500 text-sm">笔画</span>
                <p class="text-gray-900">{{ currentCharacter.strokes ? `${currentCharacter.strokes} 画` : '-' }}</p>
              </div>
              <div>
                <span class="text-gray-500 text-sm">结构</span>
                <p class="text-gray-900">{{ currentCharacter.structure || '-' }}</p>
              </div>
              <div>
                <span class="text-gray-500 text-sm">部首</span>
                <p class="text-gray-900">{{ currentCharacter.radical || '-' }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div>
        ...
        <div class="grid grid-cols-5 md:grid-cols-8 lg:grid-cols-10 gap-3">
          ...
          >
            <span class="text-2xl font-bold text-gray-900">{{ char.word }}</span>
          </RouterLink>
        </div>
      </div>
    </template>
