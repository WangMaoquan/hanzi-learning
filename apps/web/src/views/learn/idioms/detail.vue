...
<template>
  ...
  <div v-else-if="idiom">
    ...
    <div class="bg-white rounded-xl p-8 shadow-sm border border-gray-100">
      ...
      <div class="text-center mb-8">
        <h2 class="text-5xl font-bold text-gray-900 mb-2">{{ idiom.word }}</h2>
        <p class="text-xl text-gray-500">{{ idiom.pinyin }}</p>
      </div>

      <!-- 释义 -->
      <div class="mb-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-2">释义</h3>
        <p class="text-gray-700 leading-relaxed">{{ idiom.explanation || '暂无释义' }}</p>
      </div>

      <!-- 出处 -->
      <div v-if="idiom.derivation" class="mb-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-2">出处</h3>
        <p class="text-gray-600 italic">{{ idiom.derivation }}</p>
      </div>

      <!-- 例句 -->
      <div v-if="idiom.example" class="mb-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-2">例句</h3>
        <p class="text-gray-700 italic bg-gray-50 p-4 rounded-lg">{{ idiom.example }}</p>
      </div>
  ...
  <div v-else class="text-center py-12">
    ...
    <RouterLink to="/learn/idioms" class="text-primary-500 hover:underline">
      返回列表
    </RouterLink>
  </div>
</template>
