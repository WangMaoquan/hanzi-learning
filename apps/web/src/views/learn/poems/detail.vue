...
<template>
  <div v-if="poem">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-900">{{ poem.title }}</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <!-- 赏析 -->
        <div v-if="poem.appreciation" class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          <h3 class="font-semibold text-gray-900 mb-3">赏析</h3>
          <p class="text-gray-600 leading-relaxed">{{ poem.appreciation }}</p>
        </div>

        <!-- 创作背景 -->
        <div v-if="poem.background" class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
      ...
      <div class="space-y-6">
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          <div v-if="poem.annotation" class="space-y-3">
            <div
              v-for="(value, key) in poem.annotation"
              :key="key"
              class="flex gap-2"
            >
  ...
  <div v-else class="text-center py-12">
    ...
    <RouterLink to="/learn/poems" class="text-secondary-500 hover:underline">
      返回列表
    </RouterLink>
  </div>
</template>
